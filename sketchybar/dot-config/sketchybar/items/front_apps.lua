...
local function updateWindows(windows)
    ...
    for window in foundWindows do
        ...
        frontApps[windowName] =
            sbar.add(
            "item",
            constants.items.FRONT_APPS .. "." .. windowName,
            {
                label = {
                    padding_left = 0,
                    padding_right = 0
    ...
    sbar.exec(
        constants.aerospace.GET_CURRENT_WINDOW,
...
frontAppWatcher:subscribe(
    constants.events.UPDATE_WINDOWS,
    function()
        getWindows()
    end
)
